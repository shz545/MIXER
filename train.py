...
class Trainer(object):
    def __init__(self, model, args):
        wandb.config.update(args)                             # 上傳設定參數到 wandb
        self.device = args.device                             # 設定運算裝置
        self.clip_grad = args.clip_grad                       # 梯度裁剪
        self.cutmix_beta = args.cutmix_beta                   # CutMix β值
        self.cutmix_prob = args.cutmix_prob                   # CutMix 機率
        self.model = model                                    # 傳入的模型
    ...
    def _train_one_step(self, batch):
        ...
        if self.cutmix_beta > 0 and r < self.cutmix_prob:
            ...
            lam = np.random.beta(self.cutmix_beta, self.cutmix_beta) # 產生 λ
            rand_index = torch.randperm(img.size(0)).to(self.device) # 隨機排序
            target_a = label
            target_b = label[rand_index]
            bbx1, bby1, bbx2, bby2 = rand_bbox(img.size(), lam)
            img[:, :, bbx1:bbx2, bby1:bby2] = img[rand_index, :, bbx1:bbx2, bby1:bby2] # 替換 patch
            # adjust lambda to exactly match pixel ratio
            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (img.size()[-1] * img.size()[-2]))
            # compute output
            with torch.cuda.amp.autocast(): # 使用 AMP
                out = self.model(img)   # 模型預測
                loss = self.criterion(out, target_a) * lam + self.criterion(out, target_b) * (1. - lam)   # 計算 CutMix 或一般損失
        else:
            # compute output
        ...
        self.scaler.scale(loss).backward() # 放大 loss，回傳
        if self.clip_grad:
            nn.utils.clip_grad_norm_(self.model.parameters(), self.clip_grad) # 梯度裁剪
        self.scaler.step(self.optimizer) # 更新權重
        self.scaler.update() # 更新 AMP scaler
